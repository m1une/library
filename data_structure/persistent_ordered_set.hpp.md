...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 1 \"data_structure/persistent_ordered_set.hpp\"\n\n\n\n#include\
    \ <functional>\n#include <initializer_list>\n#include <utility>\n#include <vector>\n\
    \n#line 1 \"data_structure/persistent_ordered_multiset.hpp\"\n\n\n\n#include <cassert>\n\
    #include <chrono>\n#include <cstdint>\n#line 9 \"data_structure/persistent_ordered_multiset.hpp\"\
    \n#include <memory>\n#line 12 \"data_structure/persistent_ordered_multiset.hpp\"\
  ...
  code: "#ifndef M1UNE_PERSISTENT_ORDERED_SET_HPP\n#define M1UNE_PERSISTENT_ORDERED_SET_HPP\
    ...
    \n\nnamespace m1une {\nnamespace data_structure {\n\ntemplate <typename T, typename\
    \ Compare = std::less<T>>\nstruct PersistentOrderedSet {\n   private:\n    using\
    \ Multiset = PersistentOrderedMultiset<T, Compare>;\n\n    Multiset data;\n\n\
...
documentation_of: data_structure/persistent_ordered_set.hpp
layout: document
...
* `PersistentOrderedSet clear() const`
  Returns an empty set with the same comparator and random state. ($O(1)$)

* `PersistentOrderedSet insert(T key) const`
  Returns a new set with `key` inserted. If `key` already exists, returns an equivalent set. ($O(\log N)$)

* `PersistentOrderedSet erase(const T& key) const`
  Returns a new set with `key` removed if it exists. ($O(\log N)$)
...
int main() {
    m1une::data_structure::PersistentOrderedSet<int> a;
