...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 1 \"data_structure/dsu_monoid.hpp\"\n\n\n\n#include <algorithm>\n\
    ...
    };\n\n}  // namespace monoid\n}  // namespace m1une\n\n\n#line 12 \"data_structure/dsu_monoid.hpp\"\
...
| `T prod(int a)`, `T get(int a)` | Returns the aggregate of the set containing `a`. | $O(\alpha(N))$ amortized |
| `void set(int a, T value)` | Replaces the aggregate of the set containing `a`. | $O(\alpha(N))$ amortized |
| `std::vector<std::vector<int>> groups()` | Returns all sets as vectors of element indices. | $O(N \alpha(N))$ |

## Example

```cpp
#include "data_structure/dsu_monoid.hpp"
...
int main() {
    ...
    m1une::data_structure::DsuMonoid<Sum> dsu(std::vector<long long>{1, 2, 3, 4, 5});
