...
* `void insert(T key, int multiplicity = 1)`
  Inserts `multiplicity` copies of `key`. ($O(\log N)$)

* `bool erase_one(const T& key)`
  Removes one copy of `key`. Returns whether an element was removed. ($O(\log N)$)

* `bool erase(const T& key)`
  Alias for `erase_one(key)`. ($O(\log N)$)

* `int erase_all(const T& key)`
  Removes all copies of `key` and returns the number of removed elements. ($O(\log N)$)
